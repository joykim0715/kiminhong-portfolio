...
export function usePageLoadEntrance() {
  useEffect(() => {
    ...
    const targets = [".nav-bar", ".hero-name", ".hero-desc-line", ".hero-cta"];
    ...
    const ctx = gsap.context(() => {
      ...
      tl.to(".nav-bar", { opacity: 1, y: 0, duration: 0.45 })
        .to(".hero-name", { opacity: 1, y: 0, duration: 0.55 }, "-=0.25")
        .to(".hero-desc-line", { opacity: 1, y: 0, duration: 0.45, stagger: 0.1 }, "-=0.3")
