...
type ProjectCardProps = {
  ...
  compact?: boolean;
  /** Tighter card for a multi-card pinned grid */
  dense?: boolean;
};
...
export default function ProjectCard({
  ...
  if (compact || dense) {
    return (
      ...
      >
        <div className={`border-b border-border text-center ${dense ? "px-3 py-2.5 sm:px-4" : "px-5 py-4 sm:px-6"}`}>
          <p className="section-meta text-primary">{work.category}</p>
          <h3
            className={`text-preline mt-1 font-bold tracking-tight text-text ${
              dense ? "line-clamp-2 text-sm sm:text-base" : "mt-1.5 text-base sm:text-lg"
            }`}
          >
            {work.title}
          </h3>
        ...
        <div
          className={`relative overflow-hidden bg-dark-surf ${
            dense ? "aspect-[16/10] p-2 sm:p-2.5" : "aspect-[5/3] p-3 sm:p-4"
          }`}
        >
          <DeviceMockup>
